...
def ecb_encrypt(key, msg, IV=None):
    if(IV):
        msg = xor(msg, IV)
...
def ecb_decrypt(key, cipher, IV=None):
    _AES = AES.new(key, AES.MODE_ECB)
...
def ECB_encrypt(key, text):
    ...
    for i in range(0, len(text), C_SIZE):
        e_chunk = ecb_encrypt(key, text[i:i+C_SIZE])
        encrypted += e_chunk
    return encrypted
...
def ECB_decrypt(key, cipher):
    ...
    for i in range(0, len(cipher), C_SIZE):
        d_chunk = ecb_decrypt(key, cipher[i:i+C_SIZE])
        decrypted += d_chunk
    return decrypted
...
def CBC_encrypt(key, text):
    ...
    encrypted = bytes(IV)
    for i in range(0, len(text), C_SIZE):
        e_chunk = ecb_encrypt(key, text[i:i+C_SIZE], IV=e_chunk)
        encrypted += e_chunk
    return encrypted
...
def CBC_decrypt(key, cipher):
    ...
    for i in range(C_SIZE, len(cipher), C_SIZE):
        ...
        d_chunk = ecb_decrypt(key, cipher[i:i+C_SIZE], IV=prev_c_chunk)
        if(i != 0):
            decrypted += d_chunk
    return decrypted
